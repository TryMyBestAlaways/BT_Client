...
public class Protocol {
	...
	public static final int SET_THROTTLE = 0x01, SET_YAW = 0x02,
		   SET_PITCH = 0x03, SET_ROLL = 0x04, ARM_IT = 5, DISARM_IT = 6,
		   SET_4CON = 88, LAUCH = 8, LAND_DOWN = 9, HOLD_ALT = 10,
		   STOP_HOLD_ALT = 11, HEAD_FREE = 12, STOP_HEAD_FREE = 13,
		   POS_HOLD = 14, STOP_POS_HOLD = 15, FLY_STATE = 16,//
	// pitch、roll、yaw、Altitude、GPS_FIX?、Sat
	// num、Voltage
	MSP_SET_1WP = 17, SET_MOTOR = 214, MSP_ACC_CALIBRATION = 205;

	public static final int LAUCH_THROTTLE = 1455;
	public static final int LAND_THROTTLE = 1340;
	public static byte[] arrayData = new byte[24];
...
//                    cmdData.add((byte) ((roll) & 0xff));
//                    cmdData.add((byte) ((roll >> 8) & 0xff));
//                    for(byte b:arrayData){
//                         cmdData.add(b);
//                    }

				for (int i = 0; i < 14; i++) {
					cmdData.add(arrayData[i]);
				}
				int k = 0;
				for (int j = 0; j < 12; j++) {
					Log.w("tag", "######%%%%#####" + ((arrayData[k] & 0xff) +
						   (arrayData[++k] << 8)));
					k++;
...
//                    }
	...
	static byte[] getSendData(int cmd, byte[] data) {
		...
		byte checksum = 0;
		// byte pl_size = (byte)((payload != null ?
		// PApplet.parseInt(payload.length) : 0)&0xFF);
	...
	static int processDataIn(byte[] inData, int len) {
		...
		for (i = 0; i < len; i++) {
			...
			if (c_state == IDLE) {// $
				...
				err_rcvd = (c_state == HEADER_ERR); // now we are expecting the
				// payload size
				dataSize = (c & 0xFF);
